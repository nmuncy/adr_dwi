...
As all tracts were modeled in a single, whole-brain longitudinal model in order to account for within-subject variance, we additionally investigated the other tract smooths for concussion injury- and recovery-related FA changes that were above and beyond any variance explained by run-rerun model. This analysis identified three patterns of FA differences: injury and recovery, injury with subsequent progression, and injury without recovery. Specifically, the callosum superior frontal and right cingulum cingulate showed evidence of injury and recovery, the callosum superior parietal (as stated above), left corticospinal, right anterior thalamic, and right inferior fronto-occipital tracts progressed after injury, and the callosum motor, callosum orbitalis, left arcuate, and right uncinate showed evidence of injury and then no subsequent change.
...
Similarly, within-tract FA changes in the callosum superior frontal (Supplemental Figure \ref{supp-fig:lgio-gam-recov}) are driven entirely by RD, as indicated by the flat AD smooths. Midline increases in FA (about node 50) at Post are associated with decreased RD, and both FA and RD values return to near baseline values by RTP. This pattern is reversed in the right hemisphere (node 25), with decreased FA values being driven by increased RD. Together, these smooths reveal subtle, within-tract dynamics that vary both anatomically (anteroposterior or mediolateral) and temporally (Post or RTP relative to Base), where concussion-related increases in FA are indeed driven by RD.


\subsubsection{Evidence of Injury Progression}
\label{sssec:res-dwi-pat-prog}
Four tracts demonstrated patterns of FA change that are consistent with injury progression: the left arcuate (Figure \ref{fig:lgio-gam-prog}), callosum motor, callosum orbital, and right uncinate (Supplemental Figure \ref{supp-fig:lgio-gam-prog}). In reviewing the left arcuate FA smooths, it first appears that we did not detect any acute damage, as evidenced by the flat Post vs Base smooth (also, Table \ref{tbl:lgio-gam-tract}), but instead detected FA decline which emerged during the chronic phase (RTP) about node 65. As above, this FA decrease is associated with RD increase in the same nodal region without accompanying changes in AD. Additionally, there appears slight increases in FA towards the tract posterior end (nodes 80+) which also appear to align with RD decrease.
