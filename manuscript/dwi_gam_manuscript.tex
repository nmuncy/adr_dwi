...
From the whole-brain analysis, we identified a subset of tracts that showed concussion- and/or recovery-related scalar changes with magnitudes larger than were expected from algorithmic variance: left arcuate (lArc), left corticospinal (lCS), right anterior thalamic (raThal), right cingulum cingulate (rCCing), right inferior fronto-occipital (rIFO), and right uncinate (rUnc) (Figure \ref{fig:lgio-gam}). As an increase in FA can result from an increase in AD \textit{or} decrease in RD (and the converse for FA decreases), we next modeled FA, MD, AD, and RD for each tract, testing for Post and RTP differences from Base (R Code \ref{code:gam-lgio}). Test statistics ([REF TABLE]) indicated that all smooths differed significantly from Base in at least one region of nodes, save for the lArc Post FA smooth as well as the AD smooths (Figure \ref{fig:lgio-gam}, A \& B).

\begin{figure}[H]
	\centering
	\fbox{\includegraphics[scale=0.09]{fig_LGIO_smooths.png}}
...
Within the selected tracts, three patterns are visible. First, the lCS, raThal, and rIFO appear to have stable FA changes relative to Base. For the lCS, increases in FA (Figure \ref{fig:lgio-gam}B, node 65) are driven by decreases in RD for the same region, and this pattern is also found for rIFO (Figure \ref{fig:lgio-gam}E, node 45). For raThal, both decreases in RD and increases in AD are associated with the increase in FA values (Figure \ref{fig:lgio-gam}C, node 65). Second, the lArc and rUnc tracts both show evidence of worsening, where tract scalar become more discrepant with Base by RTP. At Post, the lArc FA values do not differ from Base (albeit the increases in MD, AD, and RD; Figure \ref{fig:lgio-gam}A), and by RTP a steep decreases in FA about node 65 is present that is driven entirely by increased RD values. Likewise, the FA values for rUnc worsen over time (Figure \ref{fig:lgio-gam}F), most dramatically in the increased FA values near node 70 which is driven by RD decreases. Finally, the rCCing (Figure \ref{fig:lgio-gam}D) shows evidence of recovery near node 35. At Post, a steep decrease in FA is associated with a corresponding increase in RD, which diminishes by RTP. That said, the slight increase in FA about node 70 at Post seems to elongate by RTP, with a decline in RD as well.
