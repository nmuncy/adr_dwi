...
As rapid acceleration and deceleration of the head results in consequential forces for sensitive neural structures, mild traumatic brain injury (mTBI) neurometabolically results in traumatic axonal injury. The physical insult causes mechanoporation of the cell membrane, resulting in an increased Ca$^{2+}$ influx which changes the membrane potential, disrupts mitochondrial oxidative metabolism, and can activate proteolytic enzymes with consequences for the structural integrity of the cell. With respect to neuronal axons, biomechanical injury and Ca$^{2+}$ influx result in multiple interrelated sequelae including: swelling (edema), axonal undulation and blebbing, neurofilament compaction, the disassembly of microtubules and other cytoskeletal structures, and potentially axotomy. Such disruptions to the cell membrane, enzymatic cascades, and axonal structure and function result in atypical water diffusion trajectories.

% DWI and mTBI
Diffusion tensor imaging (DTI) utilizes measurements of water diffusion trajectories in para-orthogonal directions to construct diffusion tensors. Within white matter, water molecules are typically constrained by the axolemma such that the principal tensor of diffusion is parallel to the axon. While orders of magnitude exist between the axonal diameter and voxel size, axonal fiber coherence results in tensors that can be used to algorithmically generate tract profiles which approximate the microstructural organization of white matter. When changes to the physical axonal structure or permeability occur, atypical diffusion can be measured with DTI and will be reflected in the respective tensors and corresponding tract profiles.

% Modeling DWI, issues resolved by GAMs
Analysis of Fiber-Tract Quantification \parencite[PyAFQ;][]{yeatman2012TractProfilesWhite,kruper2021EvaluatingReliabilityHuman,kruper2024TractometryHumanConnectome} is a software package that generates white matter tract profiles from preprocessed DWI data and is of particular relevance for mTBI research as PyAFQ subdivides white matter bundles into a number of equidistant nodes which contain scalar values for their local tract environment. As mTBI-related axonal injury is typically found within a subregion of a given tract, PyAFQ is capable of providing scalar values for the injured region that reflect injury-related changes in diffusion. Such a within-tract node approach is in contrast to the derivation of whole-tract scalar values, which are likely to be under-sensitive to within-tract changes, region-of-interest masks which may not reflect the injury locale, and skeletal analyses that may disregard meaningful variance. This increased tract resolution via PyAFQ is highly desirable for describing mTBI axonal sequelae, but a number of statistical considerations exist with such tract profiles: interdependent node-scalar values, non-linear node-scalar tract profiles, non-Gaussian scalar distributions, and multiple comparisons and their corresponding corrections.

Generalized additive models \parencite[GAMs;][]{wood2017GeneralizedAdditiveModels,pedersen2019HierarchicalGeneralizedAdditive,stasinopoulos2008GeneralizedAdditiveModels} are an extension of general linear models capable of modeling high-dimensional data which contain non-linear relationships. Where regression models fit data with a linear function, GAMs construct a smooth curve to fit data from a set of basis functions (i.e. splines; also see \cite{verbyla1999AnalysisDesignedExperiments}). Such a smooth can capture complex X-Y relationships that would be underfit by models with linear assumptions. Further, non-linear three-way relationships can be modeled via surfaces termed a `tensor product interaction smooth', and hypersurfaces can model even higher dimensional interactions \parencite{baayen2020IntroductionGeneralizedAdditive}. Such capabilities have made GAMs useful in fields such as ecology, paleontology, and linguistics \parencite[e.g.][]{simpson2018ModellingPalaeoecologicalTime,pedersen2019HierarchicalGeneralizedAdditive,schmidt2011SpatiallyExplicitHeight,wieling2011QuantitativeSocialDialectology,simpson2018ModellingPalaeoecologicalTime,wieling2018AnalyzingDynamicPhonetic,murase2009ApplicationGeneralizedAdditive}, which often model complex data in high dimensions and/or across multiple factors, and researchers using MRI techniques are beginning to adopt the statistical approach \parencite[e.g.][]{lee2025AtypicalMaturationFunctional,xu2025AgeBSASexspecific,wierenga2018UnravelingAgePuberty,mundo2022GeneralizedAdditiveModels,roy2025DevelopmentArcuateFasciculus,sorensen2021MetaanalysisGeneralizedAdditive,caffarra2024DevelopmentAlphaRhythm}. We previously demonstrated that GAMs are well-fit to model DTI tractometric profiles and provided details to guide decision points in model specification to aid their implementation \parencite{muncy2022GeneralAdditiveModels}. In that work, we used GAMs to investigate cross-sectional group tractometric differences, showcasing the sensitivity and utility of GAMs to detect subtle within-tract group differences in a principled fashion that accounted for the large number of tract nodes, the distribution of scalar values, and avoided the need for `point-wise' comparisons.

% Purpose - (a) extend GAMs for longitudinal DWI, (b) tensor product interaction smooths for multi-modal analyses
Here, we extend the use of GAMs to study sports-related concussion and subsequent recovery in a longitudinal dataset. First, we employed hierarchical GAMs to effectively conduct whole-brain, longitudinal analyses. Such an analysis is critical as injury (and recovery) may occur across multiple tracts, so pooling within-subject variance across tracts and time allows for a better model of injury; modeling tracts individually would lose such variance and increase the multiple comparison correction. Then, to better understand the nature of sport-related injury, tracts which differed were further interrogated in post-hoc analyses to determine whether the change fractional anisotropy (FA) was driven by axial or radial diffusivity. Second, we utilized tensor product interaction smooths to build multimodal models with which we tested whether changes in FA related to clinical assessment metrics. Such multimodal models helped elucidate the connection between physical injury and associated clinical assessments. We hypothesized (a) injury would result in decreased FA in the posterior corpus callosum (splenium) that would then return to baseline levels upon recovery, (b) increased radial diffusivity would primarily drive the decreased FA, reflective of axolemmal but not cytoskeletal changes conducive of subsequent recovery, and (c) ImPACT responses would relate to FA changes at injury.
