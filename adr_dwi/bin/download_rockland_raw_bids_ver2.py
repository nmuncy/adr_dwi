...
#
# Modified by: Kefin Sajan 2023
# Based on Ver1 by: Daniel Clark, John Pellman 2015/2016, Carlos G.Candano 2017/2018, Yiwen Tian 2021
...
def files(client, bucket, prefix=""):
    ...
    for result in paginator.paginate(
        Bucket=bucket, Prefix=prefix, Delimiter="participants.tsv"
    ):
...
def collect_and_download(
    ...
    if os.path.exists(aws_links):
        ...
        # participants_df = pandas.read_csv('aws_links.csv', delimiter=",", na_values=['n/a'])
        participants_df = list(csv.reader(aws_links, delimiter=","))
    ...
    if handedness == "R":
        participants_df = participants_df[
            participants_df["handedness"] == "RIGHT"
        ]
    elif handedness == "L":
        participants_df = participants_df[
            participants_df["handedness"] == "LEFT"
        ]
    participants_df = participants_df[
        participants_df["session"].isin(sessions)
    ]
    participants_df = participants_df[
        participants_df["filepath"].str.contains("|".join(scans))
    ]
    # participants_df = participants_df[participants_df['filepath'].contains('|'.join(scans))]
    participants_df = participants_df[
        (
            participants_df["filepath"].str.contains("|".join(series_filt))
            == True
        )
        | (participants_df["filepath"].str.contains("func") == False)
    ]
    # participants_df = participants_df[(participants_df['filepath'].contains('|'.join(series_filt))==True)|(participants_df['filepath'].contains('func')==False)]

    print("Passed participants_df")

    if len(participants_df) == 0:
        print(
            "No participants meet the criteria given.  No download will be initiated."
        )
        return

    # Generate a list of participants, single column list to filter on.
    participants_filt = [
        "sub-" + label + "/" for label in participants_df["subject"].tolist()
    ]
    ...
    print("Passed s3_keylist")

    # Append as well the dataset_desc json path to the end of s3_keylist
    s3_keylist.append(
        "/".join(["s3://fcp-indi", s3_prefix, "dataset_description.json"])
    )
    ...
    newParticipantsDf = pandas.DataFrame(participants_df)
    # df = pd.DataFrame(L)
    # print(participants_df)
    print(newParticipantsDf)
    for row in s3_keylist:
        rowDivided = row.split("/")
        # Get only the rows with sub-A000
        justTheSub = [
            x for x in rowDivided if x.startswith("sub-A") and len(x) == 13
        ]
        print((justTheSub))
        ...
        if len(justTheSub) > 0:
            # Remove the sub- part
            ursi = justTheSub[0][4:]
            particRow = participants_df[
                participants_df.iloc[:, 0].str.contains(ursi)
            ]
            # particRow=participants_df[participants_df.iloc[:,0].contains(ursi)]
            ...
            newParticipantsDf = pandas.DataFrame.drop_duplicates(
                newParticipantsDf
            )
    participants_df = newParticipantsDf

    print(participants_df)
    print("Passed merging to ParticipantsDf")
        ...
        try:
            if not os.path.exists(download_file):
                ...
                if dryrun:
                    ...
                    # print('File '+float(path_idx+1)+'out of '+total_num_files)
            else:
                print(
                    "File {} already exists, skipping...".format(download_file)
                )
                files_downloaded -= 1
        except Exception as exc:
            print(
                "There was a problem downloading %s.\n"
                "Check input arguments and try again." % s3_path
            )
            print(exc)

    print("Passed files_downloaded")

    if dryrun:
        print(
            "%d files would be downloaded for %d participant(s)."
            % (files_downloaded, len(participants_df))
        )
    else:
        print(
            "%d files downloaded for %d participant(s)."
            % (files_downloaded, len(participants_df))
        )
    if not dryrun:
        ...
        if os.path.isfile(os.path.join(out_dir, "participants.tsv")):
            old_participants_df = pandas.read_csv(
                os.path.join(out_dir, "participants.tsv"),
                delimiter="\t",
                na_values=["n/a", "N/A"],
            )
            participants_df = participants_df.append(
                old_participants_df, ignore_index=True
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=False,
        type=str,
        help="Path to aws_links.csv. Leave it empty "
    ...
    parser.add_argument(
        ...
        required=False,
        type=float,
        help="Upper age threshold (in years) of "
    ...
    parser.add_argument(
        ...
        required=False,
        type=float,
        help="Lower age threshold (in years) of "
        "particpants to download (e.g. for "
    ...
    parser.add_argument(
        ...
        required=False,
        type=str,
        help="Participant handedness to download only " "(e.g. 'R' or 'L')",
    )
    parser.add_argument(
        "-v",
        "--sessions",
    ...
    if args.handedness:
        ...
        if kwargs["handedness"] == "R":
            ...
            print(
                "Input for handedness '%s' was not 'L' or 'R'."
                % kwargs["handedness"]
            )
            print("Please check script syntax and try again.")
            sys.exit(1)
...
# /Users/ksajan/opt/anaconda3/bin/python /Users/ksajan/Downloads/webTest/download.py
