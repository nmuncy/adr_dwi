...
\textit{Corpus Callosum Results}. The corpus callosum is divided into eight main tracts by PyAFQ: anterior forceps (CCaf), motor (CCmot), occipital (CCocc), orbital (CCorb), posterior parietal (CCpp), superior frontal (CCsf), superior parietal (CCsp), and temporal (CCtemp). We investigated whether these tracts would have concussion- and recovery-related scalar changes, with particular interest in the posterior regions (CCsp, CCpp). As an increase in FA can result from an increase in AD \textit{or} decrease in RD, we next modeled FA, MD, AD, and RD for each tract, testing for Post and RTP differences from Base (R Code \ref{code:gam-lgio}).

Figure \ref{fig:lgio-gam-cc} illustrates the scalar smooths for each of these tracts, including smooths for both the global tract profile and Post and RTP differences from Base. Inspection of these smooths reveals a number of patterns. First, despite significant test statistics against flatness, four tracts show relatively little FA change from Base: CCaf, CCocc, CCpp, and CCtemp (Figure \ref{fig:lgio-gam-cc}, A, C, E, \& H); such small difference magnitudes give little evidence of practical significance. Second, the Post-Base and RTP-Base FA differences appear to be constant for the CCsp (Figure \ref{fig:lgio-gam-cc}, G), where FA values about nodes 50-60 have increased relative to Base, seemingly driven by a decreased RD. Such a pattern recovery is consistent with axonal edema, particularly given the lack of change in AD. Third, evidence of recovery is apparent in the CCsf (Figure \ref{fig:lgio-gam-cc}, F). At Post, CCsf FA values around node 50 are elevated with respect to Base, but this magnitude of change is decreased by RTP. A similar pattern is also found in RD about the same nodes, and this pattern is again consistent with axonal swelling at the time of injury, but with the added evidence of decreased edema by the time of RTP. Finally, two tracts appear to worsen between Post and RTP: the CCmot and CCorb (Figure \ref{fig:lgio-gam-cc}, B \& D). As with the other tracts, the CCmot appears to have inflated FA values relative to Base about node 50, driven by decreased RD. Instead of constancy or recovery, however, the difference in scalar values spreads rightward (nodes 40-50). Conversely, CCorb has a decrease in FA values around node 60 (and increased RD) in Post relative to Base, a difference which increases in magnitude by RTP.


\begin{sidewaysfigure}
	\centering
	\fbox{\includegraphics[scale=0.085]{fig_LGIO_callosal_smooths.png}}
	\caption{HGAMs fitting the global (top) and session difference from Base (Post, RTP; middle, bottom) smooths. Each subplot (A-H) consists smooths for FA (top left), MD (top right), AD (bottom left), and RD (bottom right). `Diff: Post-Base' = Post difference relative to Base. `Diff: RTP-Base' = RTP difference relative to base. Red boxes indicate nodes where smooths differ statistically from the reference group (Base). Group smooths are plotted in the domain of the global smooth to show their fit contribution. LGIO = Longitudinal HGAM with Global and group (I) smooths, and an Ordered group factor. High-resolution version available online.}
...
\begin{figure}[H]
	...
	\caption{Longitudinal HGAM smooths modeling DWI scalar as a function of node. Each model fit both the global tractometric profile (global smooths) and how sessions differed from the global (group smooths). Specifically, session was treated as an ordered factor so resulting session smooths are a `difference from Base' smooth. For example, left arcuate (A) Post FA values did not differ from Base while RTP had lower FA values than base around node 65. Each section (A-F) consists of global and session smooths for FA (top left), MD (top right), AD (bottom left), and RD (bottom right). Red boxes indicate nodes where smooths differ statistically from the reference group (Base). Group smooths are plotted in the domain of the global smooth to show their fit contribution. LGIO = Longitudinal HGAM with Global and group (I) smooths, and an Ordered group factor. High-resolution version available online.}
	\label{fig:lgio-gam-sel}
\end{figure}

Within the selected tracts, and as with the corpus callosum, three patterns are visible. First, the lCS, raThal, and rIFO appear to have stable FA changes relative to Base. For the lCS, increases in FA (Figure \ref{fig:lgio-gam-sel}B, node 65) are driven by decreases in RD for the same region, and this pattern is also found for rIFO (Figure \ref{fig:lgio-gam-sel}E, node 45). For raThal, both decreases in RD and increases in AD are associated with the increase in FA values (Figure \ref{fig:lgio-gam-sel}C, node 65). Second, the lArc and rUnc tracts both show evidence of worsening, where tract scalar become more discrepant with Base by RTP. At Post, the lArc FA values do not differ from Base (albeit the increases in MD, AD, and RD; Figure \ref{fig:lgio-gam-sel}A), and by RTP a steep decreases in FA about node 65 is present that is driven entirely by increased RD values. Likewise, the FA values for rUnc worsen over time (Figure \ref{fig:lgio-gam-sel}F), most dramatically in the increased FA values near node 70, an increase which is driven by RD decreases. Finally, the rCCing (Figure \ref{fig:lgio-gam-sel}D) shows evidence of recovery near node 35. At Post, a steep decrease in FA is associated with a corresponding increase in RD, which diminishes by RTP. That said, the slight increase in FA about node 70 at Post seems to elongate by RTP, with a decline in RD as well.
...
% TODO: Motivate ImPACT metric selections
% TODO: LGIO_intx GAMs

% \begin{figure}[H]
% 	\centering
% 	\fbox{\includegraphics{fig_LGIO_intx_lCS_rx_time.png}}
% 	\caption{Caption.}
% 	\label{fig:lgio-imp-lcs}
% \end{figure}
...
% TODO: DI_time GAM results - lCS node 45.

% \begin{figure}[H]
% 	\centering
% 	\fbox{\includegraphics[scale=0.75]{fig_DI_time_lCS_fa.png}}
% 	\caption{Caption.}
% 	\label{fig:di-time-lcs}
% \end{figure}
