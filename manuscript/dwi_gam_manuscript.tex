...
Traumatic brain injury affects [XXX] individuals each year with approximately [XXX] results from sport-related activities, a number which may be under-reported.

% mTBI sequelae description
As rapid acceleration and deceleration of the head result in consequential forces for sensitive neural structures, mild traumatic brain injury (mTBI) neurometabolically results in traumatic axonal injury. Physical insult results in mechanoporation of the cell membrane, where increased Ca$^{2+}$ influx changes the membrane potential, disrupts mitochondrial oxidative metabolism, and can activate proteolytic enzymes. With respect to axons, biomechanical injury and Ca$^{2+}$ influx results in multiple interrelated sequelae including swelling (edema), axonal undulation and blebbing, neurofilament compaction, the disassembly of microtubules and other cytoskeletal structures, and potentially axotomy. Such axonal disruptions result in atypical water diffusion trajectories.
...
% Modeling DWI, issues
A number of DTI software packages exist that are capable of generating tract profiles. PyAFQ is one of particular interest as it generates a tract profile that is subdivided into equidistant nodes which contain scalar values for their local tract environment. As mTBI traumatic axonal injury is typically found within a certain subregion of a tract, PyAFQ may supply node scalar values for the injured region which reflect the change in diffusion, where whole-tract values are likely to be under-sensitive and region-of-interest masks may not reflect the injury locale. This increased tract resolution is highly desirable for describing mTBI axonal sequelae, but a number of statistical considerations exist with such tract profiles: interdependent node scalar values, non-linear node-scalar tract profiles, non-Gaussian scalar distributions, and multiple comparisons and their corresponding corrections.

We previously demonstrated that generalized additive models (GAMs) are well-fit to model such tractometric profiles and provided details to guide decision points in model specification \parencite{muncy2022GeneralAdditiveModels}. While further details are supplied below, GAMs can fit non-linear interactions through the use of splines, and as an extension of general linear models support a number of distributions that are relevant for scalar values (e.g. beta).
