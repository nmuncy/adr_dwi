...
\label{sec:disc}
% Recap
Sixty-nine NCAA athletes were recruited from the University of Nebraska-Lincoln's men's football and women's soccer programs in order to study the effects of concussion on white matter organization. Participants contributed diffusion MRI and ImPACT assessment data at three time points: pre-season (Base), within 48 hours after diagnosed concussion (Post), and after completing the return-to-play protocol (RTP). This prospective, longitudinal dataset is singular, to our knowledge, and facilitates an important step forward in understanding the nature of injury and recovery. Probabilistic tractography generated a set of white matter tracts for each subject and time point, and we employed hierarchical generalized additive models (HGAM) to (a) conduct a longitudinal, whole-brain analysis of injury and recovery, (b) determine the source of FA change, and (c) relate changes in FA values to ImPACT and recovery time. Importantly, this is the first time HGAMs have been used to study concussion- and recovery-related tract scalar changes, and we feel that such a technique is critical given their capability of modeling non-linear interactions.

% Findings
This study had a number of findings. First, we detected within-tract FA changes in hypothesized regions (posterior corpus callosum) which recovered after injury, confirming our first hypothesis. Importantly, we also detected a number of FA changes in other tracts and demonstrated that, in accordance with our second hypothesis, RD was the source of such FA change. Identifying the source of FA change is critical to understanding injury sequelae and recovery, as concussion-related changes in AD and RD are associated with different axonal pathologies. Briefly, an axon is composed of an axolemma that is anchored to the actin-spectrin complex by diverse proteins such as ankyrin, NMDA receptors, voltage-gated Na$^+$ channels (NaV), and Na$^+$/H$^+$ exchangers. Within the axon, $\alpha$/$\beta$-tubulins form microtubules which are connected with microtubule-associate proteins such as tau. These microtubules form the backbone of the axonal transport system, where transport proteins move mitochondria, vessicles, and other organelles along the axon. Neurofilaments occupy the space between the central microtubules and the actin-spectrin complex and their sidearms form connections with neighboring filaments, supporting the diameter of the axon. Externally, myelin loops sheathe the axon by concentric loops which attach to the cytoskeleton with anchoring proteins such as ankyrin, Caspr, and contactin. Rapid application of shear, tensile, and/or compressive strains which result from sports-related de/acceleration injury can have differential effects on the various components of the axon. Initially, physical deformation can result in axonal undulations, breaking microtubules, and mechanoporation of the axolemma, increasing intracellular Ca$^{2+}$ concentrations ([Ca$^{2+}$]i). Increased [Ca$^{2+}$]i has then been implicated in activating secondary injury mechanisms.
